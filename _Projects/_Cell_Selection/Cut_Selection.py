...
NOTE only body cells, and only cut-only. 
...
for _, cloc in tqdm(enumerate(msb_sites), total=len(msb_sites)):
    ...
    if a.stimset == 'Metamer_Cut_v251011':
        metamers = selected_resps[:,300:,:] # keep all except for fob.
    elif a.stimset == 'Mega_Metamer_v251104': # remove cut-shuffle parts, keep cut only.
        # metamer_cutshuffle = body_resps[:,300:2240,:]
        metamers = selected_resps[:, np.r_[300:1300, 1400:1440, 2240:3680], :]
    else:
...
#%% save all msb cells
np.savez_compressed(
    ot.Join(save_path, f'{target_area}_Cells_Metamer_Cut_Only.npz'),
