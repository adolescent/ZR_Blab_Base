...
'''
Codes for standarization operation to get Metamer Single Bubble stimuli from each site class.
...
Stimulus layout (Metamer_Singlebubble_v251107 stimset, 4540 total in recording):
  - FOB STI150 tuning block: indices 0–299 (exported separately via FOB pass)
  - Response block (exported): indices 300–4539 (4240 images)
      [0:1000]     Metamer shuffle block (stim 300–1299)
      [1000:1040]  Metamer tail (stim 1300–1339)
      [1040:2640]  Bubble / occluded block (stim 1340–2939)
      [2640:4240]  Rested block (stim 2940–4539)
'''
...
savepath = r'E:\#Preprocessed_Data\Selected_Cells\Metamers\Metamer_SingleBubble'
...
def plot_fob_heatmap(fob_avr, fob_valid_len, out_path, n_fob_max=N_FOB_MAX):
    """Per-neuron z-score heatmap; only valid FOB columns shown (150), rest masked."""
    ...
    ax.set_ylabel('Neuron')
    ax.set_title(f'FOB response — {x_norm.shape[0]} cells (STI150)')
    fig.tight_layout()
    fig.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
...
def export_stim_layout(save_root=savepath):
    ...
    data_ids = np.asarray(stim_infos[STIMSET_NAME]['Data'], dtype=np.int32)
    tsv_info, _, _ = Load_Info(STIMSET_NAME)
    if tsv_info is None:
        raise FileNotFoundError(f'{STIMSET_NAME}.tsv not found in Info_Files')
    ...
    stim_type[SLICE_METAMER_TAIL] = 'metamer_tail'
    stim_type[SLICE_BUBBLE] = 'bubble'
    stim_type[SLICE_REST] = 'rest'

    metamer_img_index = np.full(len(data_ids), np.nan, dtype=np.float32)
    metamer_shuffle_level = np.full(len(data_ids), np.nan, dtype=np.float32)
    metamer_img_index[SLICE_METAMER] = np.tile(np.arange(1, 41), 25)
    metamer_shuffle_level[SLICE_METAMER] = np.tile(np.repeat(np.arange(5), 40), 5)
    ...
    np.savez_compressed(
        ...
        object_id=obj_id,
        stim_type=stim_type,
        metamer_img_index=metamer_img_index,
        metamer_shuffle_level=metamer_shuffle_level,
        slice_metamer=np.array(SLICE_METAMER.indices(N_IMG), dtype=np.int32),
        slice_metamer_tail=np.array(SLICE_METAMER_TAIL.indices(N_IMG), dtype=np.int32),
        slice_bubble=np.array(SLICE_BUBBLE.indices(N_IMG), dtype=np.int32),
        slice_rest=np.array(SLICE_REST.indices(N_IMG), dtype=np.int32),
        n_img=np.int32(N_IMG),
        n_metamer=np.int32(SLICE_METAMER.stop),
        n_bubble=np.int32(SLICE_BUBBLE.stop - SLICE_BUBBLE.start),
        n_rest=np.int32(SLICE_REST.stop - SLICE_REST.start),
    )
    print(f'stim_layout saved -> {out_path}')
    return out_path


def export_readme(save_root=savepath):
    """Copy README template to save_root."""
    src = ot.Join(os.path.dirname(os.path.abspath(__file__)), 'README_Metamer_SingleBubble.md')
    dst = ot.Join(save_root, 'README_Metamer_SingleBubble.md')
    if not os.path.exists(src):
        print(f'README template missing: {src}')
        return None
    ot.Mkdir(save_root)
...
if RUN_SITE_REFRESH:
    warnings.filterwarnings('ignore', message='.*constant.*')
    _bubble_paths = site_index[site_index['stimset'] == STIMSET_NAME]['path'].tolist()
    for c_site in tqdm(_bubble_paths, desc='refresh Metamer_Bubble'):
        try:
            SRS = JL.load(c_site)
...
#%% export stimulus layout and README (shared, run once)

export_stim_layout(savepath)
export_readme(savepath)
...
for cloc in BRAIN_AREAS:
    ...
    for c_site in tqdm(sites, total=len(sites), desc=cloc):
        ...
        raw = SRS.raw_psth[selected][:, :, data_ids, :]  # (n, n_repeat, 4240, 450)
    ...
    plot_heatmap(avr_rsp, ot.Join(out_dir, 'heatmap_4240.png'))
