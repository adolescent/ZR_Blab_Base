'''
这个脚本旨在寻找VGG19模型中，是否存在特征性的维度，表征metamer打乱的程度，大致思路如下：

1. 首先，需要加载VGG19模型，并提取特征。
2. 做样本空间构建，使用NSD1k数据，last conv特征（fc1前一层），PCA降维到50D，展示解释的VAR和PC1-2载荷最高/最低的图片
3. 将metamer图片嵌入上述样本空间，并得到这些图片在空间中的载荷
4. 可视化，展示不同level的shuffle图片在这些PC中的载荷，以及寻找一个2D平面，是否能反映图片的打乱程度
'''


#%% 配置

savepath = r'E:\#Preprocessed_Data\Selected_Cells\Metamers\Analysis\vgg19_space'
nsd_figpath = r'E:\#Stimsets\NSD1000'
metamer_img_path = r'E:\#Preprocessed_Data\Selected_Cells\Metamers\Stimsets\Metamer1k'

FEATURE_LAYER = 'fc1'   # 'last_conv' | 'fc1'
N_DIM = 50          # 保留前 50 个主成分
BATCH = 32
N_EXTREME = 10      # PC 上载荷最高/最低各展示几张图

assert FEATURE_LAYER in ('last_conv', 'fc1'), "FEATURE_LAYER must be 'last_conv' or 'fc1'"
FEATURE_TAG = 'last_conv_pre_relu' if FEATURE_LAYER == 'last_conv' else FEATURE_LAYER
CACHE = savepath + rf'\vgg19_nsd1k_step1_{FEATURE_TAG}.npz'

SAVE_FIGURES = True
SHOW_FIGURES = True


#%% 1. 用 NSD1k + VGG19 建立 50D 样本空间
...
if os.path.isfile(CACHE):
    ...
    elif FEATURE_LAYER == 'fc1' and 'fc1' in d.files:
        feats = d['fc1']         # 旧版缓存
    else:
        raise KeyError(f'no feature array in {CACHE}')
    if FEATURE_LAYER == 'fc1' and feats.min() >= -1e-6:
        raise ValueError(
            f'{CACHE} looks like post-ReLU fc1 (min={feats.min():.4f}); '
    ...
    def _extract(batch_tensor):
        if FEATURE_LAYER == 'last_conv':
            # features[34] = conv5_4；hook 里立刻拷贝，避免后面的 inplace ReLU 改掉 pre-ReLU 输出。
            buf = []

            def _hook(m, inp, out):
                buf.append(out.detach().cpu().clone())

            h = model.features[34].register_forward_hook(_hook)
            model.features(batch_tensor)
            h.remove()
            return buf[0].flatten(1)
        # fc1: classifier[0] 线性层输出 → 4096D
        buf = []

        def _hook(m, inp, out):
            # 必须立刻拷贝：classifier[1] 是 inplace ReLU，会原地改掉 fc1 的输出张量
            buf.append(out.detach().cpu())

        h = model.classifier[0].register_forward_hook(_hook)
        model(batch_tensor)
        h.remove()
        return buf[0]
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(img_paths), BATCH), desc=f'VGG19 {FEATURE_LAYER}'):
...
#%% --- 1) 50 个 PC 解释的方差 ---
layer_label = 'last conv' if FEATURE_LAYER == 'last_conv' else 'fc1'
print(f'\n=== PCA variance (VGG19 {layer_label}, NSD1k, K={N_DIM}) ===')
...
for plot_pc in (1, 2):
    ...
    fig.suptitle(f'VGG19 {layer_label} — NSD1k extremes on PC{plot_pc} ({ev_ratio[pc_idx]:.1%} var)', fontsize=10)
    fig.tight_layout()
...
ax.set_title(f'VGG19 {layer_label} PCA (NSD1k)')
fig.tight_layout()
...
ax.set_title(f'VGG19 {layer_label} object space (NSD1k, N={len(pc1)})')
...
N_METAMER = 1000
CACHE2 = savepath + rf'\vgg19_metamer1k_step2_{FEATURE_TAG}.npz'
...
STEP2_PC12_COLOR_MODE = 'object_rainbow'   # 'ani_shuffle' | 'object_rainbow'
...
ax.set_xlabel(f'PC1 ({ev_ratio[0]:.1%} var)')
ax.set_ylabel(f'PC2 ({ev_ratio[1]:.1%} var)')
ax.set_title(f'VGG19 {layer_label} — metamer1k in NSD1k space\n{step2_title_suffix}')
ax.set_aspect('equal', adjustable='box')
fig.tight_layout()
...
STEP4_SUBSPACE_COLOR_MODE = 'shuffle_level'   # 'shuffle_level' | 'object_shuffle'
...
ax.set_title(f'VGG19 {layer_label} — 2D shuffle subspace\n{step4_title_suffix}')
ax.set_aspect('equal', adjustable='box')
...
ax.set_title(f'VGG19 {layer_label} — 1D shuffle axis load (boxplot whis 10-90)')
ax.legend(frameon=False, fontsize=8, loc='best')
fig.tight_layout()
...
ax.set_ylabel('PC')
ax.set_title(f'VGG19 {layer_label} — metamer loads on first {N_DIM} PCs')
