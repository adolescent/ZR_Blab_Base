...
#%% Face PSTH: avg 20 objs × 4 cond (Tex_CTR, Shading_CTR, Tex, Shading)
...
face_psth = psth_avr[:, 80:160, :][:, :, msk].reshape(n_cell, 20, 4, -1)
face_cond = face_psth.mean(1)                               # avg over 20 face images

n_t = face_cond.shape[-1] // bin_ms * bin_ms
t_plot = t_ms[msk][:n_t].reshape(-1, bin_ms).mean(1)
face_fr = face_cond[..., :n_t].reshape(n_cell, 4, -1, bin_ms).mean(-1) * 1000  # Hz
...
ax.set_title(f'Face response of ML neurons')
