'''
This script will select Bubble Selection .

mostly the same code as only metamer part.
'''
...
if (target_area == 'ASB') or (target_area == 'AL'):
    msb_sites = ot.Get_File_Name(r'E:\#Preprocessed_Data\SiteClass\anagram_jigsaw','.joblib')
else:
    raise ValueError(f'{target_area} is not a valid target area.')
...
for _, cloc in tqdm(enumerate(msb_sites), total=len(msb_sites)):
    ...
    if a.stimset == 'Anagram_Jigsaw_v260227':
        metamers = selected_resps[:,150:, :]
    ...
    if expected_fob_n is None and fob_resp.shape[1] > 0:
        ...
        print('Warning of FOB: Not STI 150. Fill with 150.')
...
#%% save all msb cells
np.savez_compressed(
    ot.Join(save_path, f'{target_area}_Cells_Ana_Jig.npz'),
...
#%% counts 
counter = 0
acs = all_d_primes[all_d_primes.Category=='Body'].reset_index(drop=True)
