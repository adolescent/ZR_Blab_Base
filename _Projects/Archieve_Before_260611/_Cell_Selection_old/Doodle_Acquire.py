...
if (target_area == 'ASB') or (target_area == 'AL'):
    msb_sites = ot.Get_File_Name(r'E:\#Preprocessed_Data\SiteClass\Doodle\ASB','.joblib')
else:
    msb_sites = ot.Get_File_Name(r'E:\#Preprocessed_Data\SiteClass\Doodle\MSB','.joblib')
...
for _, cloc in tqdm(enumerate(msb_sites), total=len(msb_sites)):
    ...
    all_metamer_resp_list.append(doodles)
...
#%% save all msb cells
np.savez_compressed(
    ot.Join(save_path, f'{target_area}_Cells_Doodle.npz'),
