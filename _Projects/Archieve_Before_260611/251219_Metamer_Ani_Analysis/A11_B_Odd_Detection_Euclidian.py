...
- 原始图A vs 原始图B
------
和前面的几乎没有区别,但是用的是欧氏距离而不是相关。同时考虑了强度和模式。
...
for l in range(1,5):
    ...
    for j in tqdm(range(20)):
        ...
        for i,c_pair in enumerate(all_pairs):
            ...
            c_correct = softmax(dists*scale/dists.mean())[0]
            # c_correct = dists[0]/dists.sum()
            correct_rate.loc[counter,:] = [c_constrain,j,c_correct,'Alexnet']
            counter += 1 
            # vgg
            a_vgg = np.sqrt(np.sum((raw_vgg - c4_vgg)**2))
            b_vgg = np.sqrt(np.sum((raw_vgg - c42_vgg)**2))
            c_vgg = np.sqrt(np.sum((c4_vgg - c42_vgg)**2))
            dists_vgg = np.array([(a_vgg+b_vgg)/2,(a_vgg+c_vgg)/2,(b_vgg+c_vgg)/2])
            c_correct_vgg = softmax(dists_vgg*scale/dists_vgg.mean())[0]
            # c_correct_vgg = dists_vgg[0]/dists_vgg.sum()
            correct_rate.loc[counter,:] = [c_constrain,j,c_correct_vgg,'VGG19']
            counter += 1 
            # resnet
            a_res = np.sqrt(np.sum((raw_res - c4_res)**2))
            b_res = np.sqrt(np.sum((raw_res - c42_res)**2))
            c_res = np.sqrt(np.sum((c4_res - c42_res)**2))
            dists_res = np.array([(a_res+b_res)/2,(a_res+c_res)/2,(b_res+c_res)/2])
            c_correct_res = softmax(dists_res*scale/dists_res.mean())[0]
            # c_correct_res = dists_res[0]/dists_res.sum()
            correct_rate.loc[counter,:] = [c_constrain,j,c_correct_res,'Resnet18']
            counter += 1 
