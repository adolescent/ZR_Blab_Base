    ...
    if any(emptyWaveforms)
        ...
        for iCluster = 1:size(emptyWaveforms, 1)
            ...
            if param.saveMultipleRaw
                ...
                file_name =  fullfile(savePath, 'RawWaveforms', ['Unit', num2str(emptyWaveforms(iCluster)-1), '_Spike.mat']);
                wf = single(tmpspkmap(:,:,1:min(size(tmpspkmap,3), 50)));
                save(file_name, 'wf');
                writeNPY((tmpspkmap(:,:,1:min(50, size(tmpspkmap,3)))), fullfile(savePath, 'RawWaveforms', ['Unit', num2str(emptyWaveforms(iCluster)-1), '_RawSpikes.npy']))
